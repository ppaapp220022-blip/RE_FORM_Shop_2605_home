...
- `GET /api/community/{commId}`와 `GET /api/community/{commId}/replies`는 비로그인 호출도 허용 의도처럼 보이지만, 현재 컨트롤러는 `principal.getMemberId()`를 직접 호출한다. 비로그인 연동 전 실제 동작 확인이 필요하다.
